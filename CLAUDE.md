...
uv run pytest --cov=aicache --cov-report=term-missing          # with coverage
```

**Linting & type checking:**
```bash
uv run ruff check aicache/
uv run ruff format aicache/
uv run mypy aicache/ --ignore-missing-imports
```
...
AiCache is a multi-layer caching system for AI agent pipelines. It intercepts calls to LLMs, embedding models, vector retrievers, and agent frameworks to return cached results instead of making redundant API calls.
...
Redis, FAISS, and Chroma are optional extras installed via `pip install aicache[redis]`, `[vector-faiss]`, or `[vector-chroma]`.

### Cache Layers (`aicache/layers/`)
...
### Configuration (`aicache/config/settings.py`)

`AicacheSettings` is a dataclass with 12-factor env var support. Key variables:
- `AICACHE_BACKEND` — `memory` (default), `disk`, or `redis`
- `AICACHE_REDIS_URL`, `AICACHE_DISK_PATH`
- `AICACHE_SEMANTIC_THRESHOLD` — cosine similarity cutoff (default: 0.95)
- `AICACHE_TTL_{EMBEDDING,RETRIEVAL,CONTEXT,RESPONSE}` — per-layer TTL overrides
- `AICACHE_NAMESPACE` — key prefix (default: `aicache`)

### Public API

17 classes exported from `aicache/__init__.py`. Optional backends/adapters are imported dynamically based on installed extras — import errors for missing extras should be caught gracefully.
